...
export default function RaoMooe() {
  ...
  let breadcrumbLinks = [
    ...
    { title: 'Categories', to: '/apps/rao/mooe/categories' },
    { title: breadcrumbTitle }
  ];
  if (selectedTab === 0) {
    breadcrumbLinks = [{ title: 'Home', to: APP_DEFAULT_PATH }, { title: 'Categories' }];
  }

  useEffect(() => {
    if (pathname === '/apps/rao/mooe/categories') {
      setValue(0);
    }
    }, [pathname]);
    ...
    <>
      ...
      {user?.role !== 'guest' && (
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider', width: '100%' }}>
          <Tabs value={value} onChange={handleChange} variant="scrollable" scrollButtons="auto" aria-label="mooe tab">
            <Tab label="Categories Lists" component={Link} to="/apps/rao/mooe/categories" icon={<AppstoreOutlined />} iconPosition="start" />
            <Tab label="Budget" component={Link} to="/apps/rao/mooe/budget" icon={<AppstoreOutlined />} iconPosition="start" />
