...
export default function RaoPs() {
  ...
  let breadcrumbHeading = 'Personnel Services';
  switch (pathname) {
    case '/apps/rao/ps/categories':
      breadcrumbTitle = 'Categories';
      breadcrumbHeading = 'Categories';
      selectedTab = 0;
      break;
    case '/apps/rao/ps/budget':
      breadcrumbTitle = 'Budget';
      breadcrumbHeading = 'Budget';
      selectedTab = 1;
      break;
    case '/apps/rao/ps/expenses':
    default:
  ...
  let breadcrumbLinks = [
    ...
    { title: 'PS', to: '/apps/rao/ps/categories' },
    { title: breadcrumbTitle }
  ];
  if (selectedTab === 0) {
    breadcrumbLinks = [{ title: 'Home', to: APP_DEFAULT_PATH }, { title: 'PS' }];
  }

  useEffect(() => {
    if (pathname === '/apps/rao/ps/categories') {
      setValue(0);
    }
  }, [pathname]);

  // Add redirect effect for guests
  useEffect(() => {
    if (user?.role === 'guest') {
      setOpenModal(true);
    }
        ...
        <>
          ...
          <MainCard border={false} boxShadow>
            <Box sx={{ borderBottom: 1, borderColor: 'divider', width: '100%' }}>
              <Tabs value={value} onChange={handleChange} variant="scrollable" scrollButtons="auto" aria-label="ps tab">
                <Tab label="Categories Lists" component={Link} to="/apps/rao/ps/categories" icon={<AppstoreOutlined />} iconPosition="start" />
                <Tab label="Budget" component={Link} to="/apps/rao/ps/budget" icon={<AppstoreOutlined />} iconPosition="start" /> 
